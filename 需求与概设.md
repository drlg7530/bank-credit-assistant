...
Session记录（ES存储）：
- 每个session产生一条记录，标题用session中第一个问题作为标题
- 示例：
  {
...
功能说明：
- 历史对话按照session维度保存，每个对话产生一条记录，标题用session中第一个问题作为标题
- 点开一条历史对话记录，按session保存当前对话内容，并在左边对话框加载当时的聊天对话

实现方式：
1. Session列表显示：
   - 显示所有session记录（标题+创建时间），按创建时间倒序
   - 每个session显示为一条历史记录
   - 点击session记录时，切换到该session，并加载该session下的所有对话

2. 对话加载：
   - 点击session记录时，左侧对话框按时间顺序展示该session下的所有对话
   - 例如：user=10000，产生了3轮对话，然后没有操作session超时了，此时该对话历史记录点开后，左侧对话框会按时间顺序展示这三轮对话

3. 新对话功能：
   - 点击"新对话"按钮，清空当前session，下次查询时创建新session
